...
export function NewParishForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const slug = await uniqueSlug(supabase, form.name);
      const { error: insertErr } = await supabase.from("parishes").insert({
        name: form.name.trim(),
        slug,
      ...
      showToast(
        "Tu parroquia quedó pendiente de revisión. Un administrador la aprobará."
      );
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <section className="rounded-2xl border border-border bg-sidebar p-5">
        ...
        <p className="mt-1 text-xs normal-case text-muted-foreground">
          Buscá tu parroquia para autocompletar nombre, dirección y ubicación.
        </p>
          ...
          <ul className="mt-3 divide-y divide-border rounded-lg border border-border bg-background">
            ...
            {totalFiltered > candidates.length && (
              <li className="px-3 py-2 text-center text-xs normal-case text-muted-foreground">
                Hay {totalFiltered - candidates.length} resultados más, refiná
                tu búsqueda.
              </li>
            )}
          </ul>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field
          label="Coordenadas"
          hint="Se completan al elegir un candidato del mapa."
        >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          type="button"
          onClick={() => router.push("/parroquias")}
