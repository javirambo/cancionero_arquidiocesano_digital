...
- **Visibilidad:** `playlists.visibility ∈ {public, unlisted, private}`. **`private` está quitado de la arquitectura por ahora:** ningún flujo de creación lo asigna y el formulario no muestra un control para elegir la visibilidad, así que en la práctica toda playlist es `public` o `unlisted`. El valor `private` sobrevive en el esquema y en la RLS (que sí lo ocultaría a terceros), pero no hay forma de generar una lista totalmente privada desde la app.
- **Compartir por URL:** una playlist `public` o `unlisted` la puede ver **cualquiera que tenga la URL** `/playlists/{uuid}`, incluso sin sesión. La única diferencia entre `public` y `unlisted` es si aparece o no en listados públicos / búsquedas; en ambos casos es accesible por link. Es el **propio usuario** quien decide compartirla (copiando el link o imprimiendo el QR — CU-12); el sistema no la restringe por dueño.
- **Playlist personal (`parish_id IS NULL`):** rol `member` puede crear listas personales sin parroquia. Nacen como **`unlisted`**, es decir **no son privadas**: no aparecen en listados ni búsquedas, pero cualquiera con el link las ve. El dueño es `created_by` y es él mismo quien las comparte por la vía que quiera (link o QR). Aparecen en sus propios listados. El admin puede reasignar el `parish_id` (convertir personal → de parroquia, o viceversa).
